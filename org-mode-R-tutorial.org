...
5) dvipng program (comes with MikTeX)
...
LaTeX documents that org-mode produces to compile. You can get both of
these (plus dvipng) through the Ubuntu package texlive-full, so simply
installing the `texlive-full` package may be the easiest option if you
happen to be on Ubuntu.

For Windows users who have installed MikTeX, I had to use the MikTeX
package manager to install the following packages for LaTeX support to
work: soul, marvosysm, wasysym, wasy, zhmetrics. Install these and you
should be good to go.

For inline image support,
6) libpng, GNU/Linux users probably have this.  I found under Windows
   ...
   libpng12.dll and zlib1.dll files into my emacs-23.2\bin directory,
...
You also have to add LaTeX to the list of supported languages. You can
do that by running the following code block with C-c C-c.
